...
public class SetXpSC implements SubCommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			if (sender.hasPermission("rpg.setxp")) {
				if (args.length == 0) {
					...
					if (args[1].endsWith("L")) {
						...
						try {
							...
							xp.setLevel(xpt, add);
							PseudoRPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " levels");
							return true;
						...
						try {
							...
							xp.setXP(xpt, add);
							PseudoRPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " xp");
							return true;
						} catch (NumberFormatException e) {
					...
					if (xp != null) {
						if (args[1].endsWith("L")) {
							...
							try {
								...
								xp.setLevel(xpt, add);
								PseudoRPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " levels for " + PlayerDataController.getName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								PseudoRPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, check);
								return false;
							...
							try {
								...
								xp.setXP(xpt, add);
								PseudoRPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " xp for " + PlayerDataController.getName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								PseudoRPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, args[1]);
								return false;
				...
				PseudoRPG.message.sendPluginError(sender, Errors.NO_PERMISSION, "set xp!");
				return false;
			}
		} else {
			if (args.length == 0) {
