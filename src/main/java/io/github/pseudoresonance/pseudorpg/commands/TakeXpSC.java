	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			if (sender.hasPermission("rpg.takexp")) {
				if (args.length == 0) {
					...
					if (args[1].endsWith("L")) {
						...
						try {
							...
							PseudoRPG.message.sendPluginMessage(sender, "Removed " + add + " levels from " + xpt.getName());
							return true;
						...
						try {
							...
							xp.removeXP(xpt, add);
							PseudoRPG.message.sendPluginMessage(sender, "Removed " + add + " xp from " + xpt.getName());
							return true;
						} catch (NumberFormatException e) {
					...
					if (xp != null) {
						if (args[1].endsWith("L")) {
							...
							try {
								...
								xp.removeLevel(xpt, add);
								PseudoRPG.message.sendPluginMessage(sender, "Removed " + add + " levels to " + xpt.getName() + " for " + PlayerDataController.getName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								PseudoRPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, check);
								return false;
							...
							try {
								...
								xp.removeXP(xpt, add);
								PseudoRPG.message.sendPluginMessage(sender, "Removed " + add + " xp from " + xpt.getName() + " for " + PlayerDataController.getName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								PseudoRPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, args[1]);
								return false;
				...
				PseudoRPG.message.sendPluginError(sender, Errors.NO_PERMISSION, "take xp!");
				return false;
			}
		} else {
			if (args.length == 0) {
