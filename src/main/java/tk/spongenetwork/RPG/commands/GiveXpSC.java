	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			if (sender.hasPermission("rpg.givexp")) {
				if (args.length == 0) {
					...
					if (args[1].endsWith("L")) {
						...
						try {
							...
							xp.addLevel(xpt, add);
							RPG.message.sendPluginMessage(sender, "Added " + add + " levels to " + xpt.getName());
							return true;
						...
						try {
							...
							xp.addXP(xpt, add);
							RPG.message.sendPluginMessage(sender, "Added " + add + " xp to " + xpt.getName());
							return true;
						} catch (NumberFormatException e) {
					...
					if (xp != null) {
						if (args[1].endsWith("L")) {
							...
							try {
								...
								xp.addLevel(xpt, add);
								RPG.message.sendPluginMessage(sender, "Added " + add + " levels to " + xpt.getName() + " for " + WolfAPI.getPlayerName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								RPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, check);
								return false;
							...
							try {
								...
								xp.addXP(xpt, add);
								RPG.message.sendPluginMessage(sender, "Added " + add + " xp to " + xpt.getName() + " for " + WolfAPI.getPlayerName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								RPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, args[1]);
								return false;
				...
				RPG.message.sendPluginError(sender, Errors.NO_PERMISSION, "give xp!");
				return false;
			}
		} else {
			if (args.length == 0) {
