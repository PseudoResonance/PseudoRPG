	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			if (sender.hasPermission("rpg.setxp")) {
				if (args.length == 0) {
					...
					if (args[1].endsWith("L")) {
						...
						try {
							...
							xp.setLevel(xpt, add);
							RPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " levels");
							return true;
						...
						try {
							...
							xp.setXP(xpt, add);
							RPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " xp");
							return true;
						} catch (NumberFormatException e) {
					...
					if (xp != null) {
						if (args[1].endsWith("L")) {
							...
							try {
								...
								xp.setLevel(xpt, add);
								RPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " levels for " + WolfAPI.getPlayerName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								RPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, check);
								return false;
							...
							try {
								...
								xp.setXP(xpt, add);
								RPG.message.sendPluginMessage(sender, "Set " + xpt.getName() + " to " + add + " xp for " + WolfAPI.getPlayerName(xp.getUUID()));
								return true;
							} catch (NumberFormatException e) {
								RPG.message.sendPluginError(sender, Errors.NOT_A_NUMBER, args[1]);
								return false;
				...
				RPG.message.sendPluginError(sender, Errors.NO_PERMISSION, "set xp!");
				return false;
			}
		} else {
			if (args.length == 0) {
